...
static ok(): Errno { return Errno.new(0); }
set_ok(): void { this.tag = 0; }
is_ok(): bool { return this.tag === 0; }

    // --- missing_memory: void if tag=1

static missing_memory(): Errno { return Errno.new(1); }
set_missing_memory(): void { this.tag = 1; }
is_missing_memory(): bool { return this.tag === 1; }
}
export function unit_z(): WasiResult<Vec3, Errno>{
let rp0 = new Vec3();
let ret = wasm_glam.unit_z(changetype<usize>(rp0));return ret == 0 ? WasiResult.ok(load<Vec3>(changetype<usize>(rp0))) : WasiResult.err(new Errno(ret as u32));}


export function normalize(v: Vec3): WasiResult<Vec3, Errno>{
let rp0 = new Vec3();
let ret = wasm_glam.normalize(changetype<usize>(v), changetype<usize>(rp0));return ret == 0 ? WasiResult.ok(load<Vec3>(changetype<usize>(rp0))) : WasiResult.err(new Errno(ret as u32));}


export function mul_vec3(q: Quat, v: Vec3): WasiResult<Vec3, Errno>{
let rp0 = new Vec3();
let ret = wasm_glam.mul_vec3(changetype<usize>(q), changetype<usize>(v), changetype<usize>(rp0));return ret == 0 ? WasiResult.ok(load<Vec3>(changetype<usize>(rp0))) : WasiResult.err(new Errno(ret as u32));}
